...
const logInfo = (command, env?) ->
    ...
    switch(command)
        ...
        case 'whatis'
            ...
            (logPad '* keyword   - keyword could be any thing, you will know if it is not in the database of ghoti', 2)
        case 'fix'
            (logPad '| Command : fix [name] > show error message if it is knowned', 1)
            (logPad '| Command : fix        > try to auto fix issue', 1)
            (logPad '* example   - ghoti fix some-issue', 2)
            (logPad '* example   - ghoti fix', 2)
            (logPad '* arguments - ghoti whatis [name]', 2)
            (logPad '* name      - name could be any thing, you will know if it is not in the database of ghoti', 2)
        case 'post'
            (logPad '| Command : show post install information', 1)
            (logPad '* info      - normally you don\'t need to call this function', 2)
            (logPad '* arguments - ghoti post [type]', 2)
            (logPad '* type      - type should be a init type of ghoti init', 2)
        case 'info'
